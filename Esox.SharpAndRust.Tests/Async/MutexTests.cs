...
public class MutexTests
{
    ...
    public async Task LockAsync_WithCancellation_ReturnsError()
    {
        ...
        // Assert
        Assert.True(result.IsFailure);
        if (result.TryGetError(out var error)) Assert.Equal(ErrorKind.Interrupted, error.Kind);
    }
    ...
    public async Task Mutex_MixedSyncAndAsync_WorksCorrectly()
    {
        ...
        await Task.Delay(100);
        if (syncResult.TryGetValue(out var syncGuard)) syncGuard.Dispose();
    ...
    public async Task LockAsync_ConcurrentDisposalAndLock_HandlesRaceCondition()
    {
        ...
        if (result.IsFailure && result.TryGetError(out var error)) Assert.Equal(ErrorKind.InvalidOperation, error.Kind);
