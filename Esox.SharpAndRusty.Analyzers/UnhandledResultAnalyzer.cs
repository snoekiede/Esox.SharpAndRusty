...
public class UnhandledResultAnalyzer : DiagnosticAnalyzer
{
    ...
    private static readonly LocalizableString MessageFormatResult =
        "Result<{0}, {1}> returned by '{2}' must be used. Ignoring a Result may hide errors.";

    private static readonly LocalizableString DescriptionResult =
        "Methods returning Result<T, E> must have their return value handled. This prevents accidentally ignoring potential errors, similar to Rust's behavior.";

    private static readonly LocalizableString TitleOption = "Option value must be used";

    private static readonly LocalizableString MessageFormatOption =
        "Option<{0}> returned by '{1}' must be used. Ignoring an Option may hide missing values.";

    private static readonly LocalizableString DescriptionOption =
        "Methods returning Option<T> must have their return value handled. This prevents accidentally ignoring missing values, similar to Rust's behavior.";

    private static readonly DiagnosticDescriptor RuleResult = new(
        DiagnosticIdResult,
        TitleResult,
        MessageFormatResult,
        Category,
        ...
        true,
        DescriptionResult,
        "https://github.com/snoekiede/Esox.SharpAndRusty/wiki/ESOX1001");

    private static readonly DiagnosticDescriptor RuleOption = new(
        DiagnosticIdOption,
        TitleOption,
        MessageFormatOption,
        Category,
    ...
    private bool IsResultOrOptionType(ITypeSymbol type, out bool isResult,
