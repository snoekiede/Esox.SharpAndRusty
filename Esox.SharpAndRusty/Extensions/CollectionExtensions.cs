...
/// <summary>
///     Provides collection extension methods for working with sequences of <see cref="Option{T}" /> and
...
public static class CollectionExtensions
{
    extension<T>(IEnumerable<Option<T>> options)
    {
        ...
        public bool AllSome()
        {
            ...
            {
                if (option is Option<T>.None) return false;
            }
        ...
        /// </code>
        /// </example>
        public Option<IEnumerable<T>> Sequence()
        {
        ...
        /// </code>
        /// </example>
        public IEnumerable<T> CollectSome()
        {
            foreach (var option in options)
            {
        ...
        /// </code>
        /// </example>
        public (List<T> values, int noneCount) PartitionOptions()
        {
        ...
        /// </code>
        /// </example>
        public Option<T> FirstSome()
        {
            foreach (var option in options)
            {
                if (option is Option<T>.Some) return option;
            }
        ...
        public bool AnySome()
        {
            foreach (var option in options)
            {
                if (option is Option<T>.Some) return true;
            }
    ...
    extension<T>(IEnumerable<T> source)
    {
        ...
        /// </code>
        /// </example>
        public Result<IEnumerable<U>, E> Traverse<U, E>(
        ...
        /// </code>
        /// </example>
        public Option<IEnumerable<U>> Traverse<U>(
        ...
        /// </code>
        /// </example>
        public Either<IEnumerable<L>, R> TraverseLeft<L, R>(
            Func<T, Either<L, R>> selector)
        {
            ...
            foreach (var item in source)
            {
                ...
                else if (either is Either<L, R>.Right right) return new Either<IEnumerable<L>, R>.Right(right.Value);
            }
        ...
        /// </code>
        /// </example>
        public Either<L, IEnumerable<R>> TraverseRight<L, R>(
            Func<T, Either<L, R>> selector)
        {
            ...
            foreach (var item in source)
            {
                ...
                else if (either is Either<L, R>.Left left) return new Either<L, IEnumerable<R>>.Left(left.Value);
            }
        ...
        /// </code>
        /// </example>
        public Validation<IEnumerable<U>, E> TraverseValidation<U, E>(
            Func<T, Validation<U, E>> validator)
        {
            ...
            foreach (var item in source)
            {
                ...
                else if (validation is Validation<U, E>.Failure failure) errors.AddRange(failure.Errors);
            }

            if (errors.Count > 0) return Validation<IEnumerable<U>, E>.Invalid(errors);
        ...
        /// </code>
        /// </example>
        public Option<U> Choose<U>(Func<T, Option<U>> selector)
        {
            foreach (var item in source)
            {
                var option = selector(item);
                if (option is Option<U>.Some) return option;
            }
    ...
    extension<T, E>(IEnumerable<Result<T, E>> results)
    {
        ...
        /// </code>
        /// </example>
        public Result<IEnumerable<T>, E> Sequence()
        {
            var values = new List<T>();
        ...
        /// </code>
        /// </example>
        public IEnumerable<T> CollectOk()
        {
            foreach (var result in results)
            {
        ...
        /// </code>
        /// </example>
        public IEnumerable<E> CollectErr()
        {
            foreach (var result in results)
            {
        ...
        /// </code>
        /// </example>
        public (List<T> successes, List<E> failures) PartitionResults()
        {
        ...
        /// </code>
        /// </example>
        public Result<T, IEnumerable<E>> FirstOk()
        {
            var errors = new List<E>();

            foreach (var result in results)
            {
                if (result.TryGetValue(out var value)) return Result<T, IEnumerable<E>>.Ok(value);
        ...
        /// </code>
        /// </example>
        public Result<IEnumerable<T>, IEnumerable<E>> SequenceAll()
        {
            var values = new List<T>();
            var errors = new List<E>();
            ...
            foreach (var result in results)
            {
                if (result.TryGetValue(out var value))
                    values.Add(value);
                else if (result.TryGetError(out var error)) errors.Add(error);
            }

            if (errors.Count > 0) return Result<IEnumerable<T>, IEnumerable<E>>.Err(errors);
    ...
    extension<L, R>(IEnumerable<Either<L, R>> eithers)
    {
        ...
        /// </code>
        /// </example>
        public Either<IEnumerable<L>, R> SequenceLeft()
        {
            var values = new List<L>();
        ...
        /// </code>
        /// </example>
        public Either<L, IEnumerable<R>> SequenceRight()
        {
            var values = new List<R>();
    ...
    extension<T, E>(IEnumerable<Validation<T, E>> validations)
    {
        ...
        ///     - A list of error collections from <c>Invalid</c> validations (each invalid validation's errors as a separate
        ///     collection)
        /// </returns>
        /// <example>
        ///     <code>
        ...
        /// </code>
        /// </example>
        public (List<T> valid, List<IReadOnlyList<E>> invalid) PartitionValidations()
        {
            ...
            foreach (var validation in validations)
            {
                ...
                else if (validation is Validation<T, E>.Failure failure) invalid.Add(failure.Errors);
            }
    ...
    extension<TKey, TValue, E>(IEnumerable<Result<KeyValuePair<TKey, TValue>, E>> results) where TKey : notnull
    {
        /// <summary>
        ...
        /// </code>
        /// </example>
        public Dictionary<TKey, TValue> ToOkDictionary()
        {
            var dictionary = new Dictionary<TKey, TValue>();

            foreach (var result in results)
            {
                if (result.TryGetValue(out var kvp)) dictionary.TryAdd(kvp.Key, kvp.Value);
            }
    ...
    extension<TKey, TValue>(IEnumerable<Option<KeyValuePair<TKey, TValue>>> options) where TKey : notnull
    {
        /// <summary>
        ...
        /// </code>
        /// </example>
        public Dictionary<TKey, TValue> ToSomeDictionary()
