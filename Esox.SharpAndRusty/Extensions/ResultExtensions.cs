...
public static class ResultExtensions
{
    ...
    extension<T, E>(Result<T, E> result)
    {
        ...
        public Result<U, E> Map<U>(Func<T, U> mapper)
        {
            ...
            return result.Match(
                value => Result<U, E>.Ok(mapper(value)),
                Result<U, E>.Err
            );
        }
        ...
        public T Unwrap()
        {
            return result.Match(
                value => value,
                error => throw new InvalidOperationException($"Cannot unwrap a failure result: {error}")
            );
        }
        ...
        public T Expect(string message)
        {
            ...
            return result.Match(
                value => value,
                error => throw new InvalidOperationException($"{message}: {error}")
            );
        }
        ...
        public Result<T, E2> MapError<E2>(Func<E, E2> errorMapper)
        {
            ...
            return result.Match(
                Result<T, E2>.Ok,
                error => Result<T, E2>.Err(errorMapper(error))
            );
        }
        ...
        {
            return result.Match(
                v => EqualityComparer<T>.Default.Equals(v, value),
                _ => false
            );
        }
