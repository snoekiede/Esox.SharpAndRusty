...
/// <summary>
///     Provides convenient pattern matching helper methods for <see cref="Option{T}" /> and <see cref="Result{T, E}" />
///     types.
/// </summary>
public static class PatternMatchingHelpers
{
    ...
    extension<T, E>(Result<T, E> result)
    {
        ...
        public Result<T, E> OnFailure(Action<E> action)
        {
            ...
            if (result.TryGetError(out var error)) action(error);
            ...
            Action<E> onFailure)
        {
            ...
            if (result.TryGetValue(out var value))
                onSuccess(value);
            else if (result.TryGetError(out var error)) onFailure(error);
        ...
        public T GetValueOrThrow(Func<E, Exception> exceptionFactory)
        {
            if (exceptionFactory is null) throw new ArgumentNullException(nameof(exceptionFactory));

            if (result.TryGetValue(out var value)) return value;

            if (result.TryGetError(out var error)) throw exceptionFactory(error);
